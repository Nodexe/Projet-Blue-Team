...
Contrairement au TP1 où l'attaque se déroulait "en direct", le TP2 demande d'abord de **préparer une scène de compromission réaliste**, puis de réaliser l'enquête après coup, pour simuler un serveur piraté sans avoir assisté à l'attaque.
...
**Recommandation** : clonage de VM-CIBLE avant de commencer (VMware : clic droit → Manage → Clone), pour garder une version "propre" en cas de besoin de recommencer le scénario.
...
```bash
# Exemple classique : le binaire "find"
...
Vérifier que le bit SUID est bien actif (le `s` à la place du `x` dans les droits) :
```bash
ls -la /usr/local/bin/find_vuln
# -rwsr-xr-x 1 root root ... /usr/local/bin/find_vuln
```

**Pourquoi `find` est un bon exemple** : `find` dispose d'une option `-exec` qui permet d'exécuter n'importe quelle commande — avec le SUID actif, cette commande s'exécute avec les droits de **root**, peu importe qui lance `find_vuln`. C'est une faille SUID documentée et classique (répertoriée sur GTFOBins).
...
En forensic , on ne travaille **jamais** directement sur le disque compromis — on travaille sur une copie, pour ne jamais altérer les preuves originales.

### 2.1 Génèrer une image RAW du disque de VM-CIBLE
...
**Transférer vers VM-DOCKER pour l'analyse**, via un partage réseau VMware (dossier partagé monté en FUSE) :
```bash
sudo vmhgfs-fuse .host:/ /mnt/hgfs -o allow_other -o uid=$(id -u) -o gid=$(id -g)
```

### 2.2 Calculer un hash de l'image (preuve d'intégrité)
...
### 2.3 Identifier les partitions de l'image avec `fdisk -l`
...
### 2.4 Monter la partition en lecture seule
...
### 3.1 Explorer `/var/log/auth.log` (connexions)
...
### 3.2 Explorer `.bash_history` (commandes tapées)

```bash
cat /mnt/forensic_vmcible/home/admin1/.bash_history
```

Commandes de reconnaissance et la ligne d'exploitation SUID retrouvées.

### 3.3 Explorer `/var/log/syslog` et `auth.log` pour la création du compte
...
### 4.1 Rechercher les binaires SUID sur l'image montée
...
### 4.2 Confirmer et documenter la faille
...
### 4.3 Vérifier l'impact (le compte backdoor créé)
...
### ⚠️ Divergence de hash constatée
...
**Ce que ça signifie** : un montage `-o ro` protège uniquement la couche filesystem finale, pas les couches LVM sous-jacentes. LVM n'est pas nativement conçu pour garantir une immutabilité complète à travers toute sa pile logicielle.
